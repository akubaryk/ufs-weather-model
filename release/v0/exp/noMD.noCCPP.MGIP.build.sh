...
valid_machines=(hera orion cheyenne macosx linux wcoss_cray wcoss_phase1 wcoss_phase2 jet gaea)
...
machine=${1}
compiler=${2}
if [[ ${machine} == hera || ${machine} == orion || ${machine} == cheyenne || ${machine} == macosx || ${machine} == linux ]]; then
